...
- Identity/Auth: inicio de sesión JWT, roles Admin y Collaborator, y usuarios activos.
...
- Production requiere PostgreSQL externo, JWT configurado externamente, orígenes CORS HTTPS explícitos, y una ruta absoluta y persistente para adjuntos.
- El frontend de Production requiere `VITE_API_BASE_URL` con la URL HTTPS de la API.
...
La validación RC del mismo día pasó compilación backend sin advertencias ni errores, 57 pruebas backend focalizadas, 42 pruebas frontend, build frontend y la comprobación de migraciones sin cambios pendientes. El arranque Development de esta validación mantuvo `PilotData` deshabilitado y no reinicializó datos.
