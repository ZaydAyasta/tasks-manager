...
public sealed class TaskBlockersApiTests(PostgresApiFactory factory)
{
    ...
    public async Task Reports_multiple_blockers_and_restores_after_last_resolution()
    {
        ...
        var resolvedA = await client.PostAsJsonAsync($"/api/tasks/{task.Id}/blockers/{blockerA.Id}/resolve", new { version = blockerB!.TaskVersion });
        ...
        var resolvedB = await client.PostAsJsonAsync($"/api/tasks/{task.Id}/blockers/{blockerB.Id}/resolve", new { version = resolveA!.TaskVersion });
    ...
    public async Task Rejects_terminal_report_and_keeps_cancelled_task_cancelled_when_resolving()
    {
        ...
        detail = await client.GetFromJsonAsync<TaskResponse>($"/api/tasks/{task.Id}"); var resolve = await client.PostAsJsonAsync($"/api/tasks/{task.Id}/blockers/{blocker!.Id}/resolve", new { version = detail!.Version }); Assert.Equal(HttpStatusCode.OK, resolve.StatusCode);
    ...
    public async Task Concurrent_resolution_with_a_stale_task_version_is_rejected()
    {
        ...
        var winner = await client.PostAsJsonAsync($"/api/tasks/{task.Id}/blockers/{first.Id}/resolve", new { version = sharedVersion });
        var loser = await client.PostAsJsonAsync($"/api/tasks/{task.Id}/blockers/{second.Id}/resolve", new { version = sharedVersion });
        Assert.Equal(HttpStatusCode.OK, winner.StatusCode); Assert.Equal(HttpStatusCode.Conflict, loser.StatusCode);
    }
    private static Task<HttpResponseMessage> Report(HttpClient client, Guid taskId, Guid userId, long version, string type) => client.PostAsJsonAsync($"/api/tasks/{taskId}/blockers", new { type, description = "  Waiting  ", version });
    private static async Task<UserDetailResponse> User(HttpClient client, string email) { var response = await client.PostAsJsonAsync("/api/users", new { fullName = email, email, role = "Collaborator", password = PostgresApiFactory.DefaultPassword }); response.EnsureSuccessStatusCode(); return (await response.Content.ReadFromJsonAsync<UserDetailResponse>())!; }
    private static async Task<ProjectCreatedResponse> Project(HttpClient client, Guid id) { var response = await client.PostAsJsonAsync("/api/projects", new { name = "Project"}); response.EnsureSuccessStatusCode(); return (await response.Content.ReadFromJsonAsync<ProjectCreatedResponse>())!; }
    ...
    private static async Task<TaskResponse> Task(HttpClient client, Guid projectId, Guid stageId, Guid userId) { var response = await client.PostAsJsonAsync($"/api/projects/{projectId}/tasks", new { stageId, title = "Task", priority = "Medium"}); response.EnsureSuccessStatusCode(); return (await response.Content.ReadFromJsonAsync<TaskResponse>())!; }
}
